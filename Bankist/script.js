...
// const arrowMovementsUSD = movements.map(mov => mov * eurToUsd);
...
//   if (mov > 0) {
//     return `Movement is ${i + 1}: You deposited ${mov}`;
//   } else {
//     `Movement ${i + 1}: You wethdrew ${Math.abs(mov)}`;
//   }
// });
...
// const newOveralBalance = accounts
//   .flatMap(acc => acc.movements)
//   .reduce((acc, mov) => acc + mov, 0);
// console.log(newOveralBalance);
...
// console.log(convertTitleCase('this is a LONG title buto not too long'));
// console.log(convertTitleCase('and is anothet title with an EXAMPLE'));
...
portion and add it to the object as a new property. Do NOT create a new array, simply loop over 
the array. Forumla: recommendedFood = weight ** 0.75 * 28. (The result is in grams of food, and 
...
current > (recommended * 0.90) && current < (recommended * 1.10). Basically, the current portion 
